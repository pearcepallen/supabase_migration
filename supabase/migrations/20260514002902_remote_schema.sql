...
create or replace view "public"."brand_status_analytics" as  WITH unnested_statuses AS (
         SELECT b.id,
           ...
           FROM ((public.brands b
             JOIN public.jt_ref_brand_status jbs ON ((jbs.brands = b.id)))
             JOIN public.ref_brand_status rs ON ((rs.id = jbs.ref_brand_status)))
        ), status_categories AS (
         SELECT u.id,
            u.brand,
            u.status,
                CASE
                    WHEN (u.status = ANY (ARRAY['Active'::text, 'Priority'::text, 'Demo Program - Depricated'::text, 'SOS Program - Depricated'::text])) THEN 'Active/Healthy'::text
                    WHEN (u.status = ANY (ARRAY['Onboarding'::text, 'New to Market'::text])) THEN 'Onboarding'::text
                    WHEN (u.status = ANY (ARRAY['Sustaining (Commission)'::text, 'Commission'::text, 'Low Comm - Depricated'::text, 'Private Label - Depricated'::text, 'Special'::text])) THEN 'Commission/Special'::text
                    WHEN (u.status = ANY (ARRAY['Pause time TBD - Depricated'::text, 'Demo Request Time Off'::text, 'Pause TBD'::text, 'In Cancellation'::text])) THEN 'At Risk/Paused'::text
                    WHEN (u.status = ANY (ARRAY['Former GoodNow Vendor'::text, 'Former Demo Vendor'::text, 'Former SOS Vendor'::text])) THEN 'Former/Inactive'::text
                    WHEN (u.status = 'Prospect'::text) THEN 'Prospect'::text
                    ELSE 'Other'::text
                END AS status_category,
                CASE
                    WHEN (u.status = ANY (ARRAY['Active'::text, 'Priority'::text, 'Demo Program - Depricated'::text, 'SOS Program - Depricated'::text, 'Onboarding'::text, 'New to Market'::text, 'Sustaining (Commission)'::text, 'Commission'::text, 'Low Comm - Depricated'::text, 'Private Label - Depricated'::text, 'Special'::text])) THEN 1
                    WHEN (u.status = ANY (ARRAY['Pause time TBD - Depricated'::text, 'Demo Request Time Off'::text, 'Pause TBD'::text, 'In Cancellation'::text, 'Prospect'::text])) THEN 2
                    WHEN (u.status = ANY (ARRAY['Former GoodNow Vendor'::text, 'Former Demo Vendor'::text, 'Former SOS Vendor'::text])) THEN 3
                    ELSE 4
                END AS health_score
...
BEGIN
  RETURN EXISTS (
    ...
    WHERE u.id = user_id
      AND tmg.status = 'd4e5f6a7-0004-0004-0004-000000000002'::uuid  -- 'Active'
  );
END;
$function$
;
